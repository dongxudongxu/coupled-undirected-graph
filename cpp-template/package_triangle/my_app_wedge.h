  ...
  void IncEval(const fragment_t& frag, context_t& ctx,
    ...
    if (ctx.stage == 0) {
      ...
      ForEach(frag.InnerVertices(), [&ctx, this, &frag](int tid, vertex_t v) {
        ...
        // Group neighbors by group ID, filtering out neighbors in the same group as v
        ...
        for (auto u : ctx.complete_neighbor[v]) {
          ...
          if (group_u != group_v) {
            group_counts[group_u]++;
          }
        }

        // Calculate wedge counts
        int wedge_count = 0;
        for (const auto& [group1, count1] : group_counts) {
          for (const auto& [group2, count2] : group_counts) {
            if (group1 < group2) {
              wedge_count += count1 * count2;
            }
          }
        }
        ctx.tricnt[v] += wedge_count;
      });
